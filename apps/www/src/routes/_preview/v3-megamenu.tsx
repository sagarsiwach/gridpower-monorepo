...
/*
  Mega panel uses rounded rectangles (border-radius) — not squircles.
  Solutions render as 2x2 image-tile grid (large duotone icon + text overlay).
*/
function Rect({
  children,
  fill,
...
/*
  Mega menu prototype — applies v3 language to a real audience-led mega.
  Hover an audience tab in the main bar to open its panel.
  Panel: 3-column squircle card — sub-solutions (left), featured (middle), spotlight (right).
*/
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative" }} onMouseLeave={() => setOpen(null)}>
        <MainNav active={open} onHover={(k) => setOpen(k)} />
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              <MegaPanel audience={AUDIENCES.find((a) => a.key === open)!} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
...
/* ------------------------------------------------------------------ */
/*  Main nav with audience tabs                                        */
/* ------------------------------------------------------------------ */

function MainNav({ active, onHover }: { active: string | null; onHover: (key: string) => void }) {
  return (
    <div
      style={{
  ...
  return (
    <div className="mx-auto max-w-[1280px] px-8">
      <div style={{ filter: "drop-shadow(0 24px 48px oklch(15.3% 0.006 107.1 / 0.18))" }}>
        ...
        >
          {/* audience-specific content cross-fades on switch */}
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              ...
              <div className="grid grid-cols-12 gap-3 p-3">
                ...
                {/* spotlight — compact ink tile */}
                <div className="col-span-2">
                  <Rect fill={tokens.ink} cornerRadius={14} style={{ height: "100%" }}>
                    <div className="p-4 flex flex-col h-full">
                      ...
                      <p className="text-[15px] font-semibold tracking-[-0.015em] leading-tight mb-2" style={{ color: "#ffffff" }}>
                        {audience.spotlight.title}
                      </p>
                      <p className="text-[11px] leading-snug mb-3" style={{ color: "rgba(255,255,255,0.65)" }}>
          ...
          {/* footer — pill-shaped link buttons (static, audience-agnostic) */}
            <div
...
/* ------------------------------------------------------------------ */
/*  FooterPill — pill-shaped link button on the panel footer strip     */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  SolutionTile — image-led tile with large duotone icon + text       */
/* ------------------------------------------------------------------ */
...
function SolutionTile({ solution }: { solution: Audience["solutions"][number] }) {
  ...
  return (
    ...
    >
      {/* image area — uses real image if provided, else falls back to duotone icon */}
      <div
      ...
      >
        {image ? (
          ...
          <div className="grid place-items-center" style={{ width: "100%", height: "100%" }}>
            <Icon size={56} weight="duotone" color={tokens.ink} style={{ opacity: 0.65 }} />
          </div>
        )}
      </div>
      {/* text caption below */}
